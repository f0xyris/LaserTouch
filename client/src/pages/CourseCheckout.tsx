    ...
    switch (category) {
      ...
      default:
        return typeof course.name === 'object' ? 
          (course.name.ua || course.name.en || course.name.pl || course.name.ru || 'Unknown Course') : 
          course.name;
    }
  };
    ...
    switch (category) {
      ...
      default:
        return typeof course.description === 'object' ? 
          (course.description.ua || course.description.en || course.description.pl || course.description.ru || 'No description') : 
          course.description;
    }
  };
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              >
                {isProcessing ? (
                  ...
                  `${t?.payAmount || "Pay"} ${(course.price / 100).toLocaleString('pl-PL')} zł`
                )}
              </Button>
            </form>
          </CardContent>
  ...
  const getStripeLocale = (lang: string) => {
    switch (lang) {
      case 'pl':
        return 'pl';
      case 'uk':
        return 'en'; // Ukrainian not supported by Stripe, use English instead
      case 'en':
      default:
        return 'en';
    }
